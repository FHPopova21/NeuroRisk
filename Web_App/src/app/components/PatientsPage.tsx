    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-slate-900">
            {isAdmin ? "Глобален мониторинг на пациенти" : "База данни с пациенти"}
          </h1>
          <p className="text-slate-500 font-medium">
            {isAdmin 
              ? "Достъп само за четене до всички регистрирани пациенти в мрежата." 
              : "Управлявайте и наблюдавайте всички свои пациенти на едно място."}
          </p>
        </div>
        {!isAdmin && (
          <Link 
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm">
        ...
        <div className="relative flex-1 md:max-w-md w-full">
          ...
          <input 
            type="text" 
            placeholder="Търсене по име, възраст или риск..." 
            value={searchTerm}
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Име</th>
                <th className="px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Възраст</th>
                <th className="px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Приложение</th>
                <th className="px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Последен анализ</th>
                <th className="px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Ниво на риск</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredPatients.map((patient, i) => (
                ...
                >
                  ...
                  <td className="px-6 py-5 text-sm font-bold text-slate-600">{patient.birth_date || "Н/Д"}</td>
        ...
        <div className="p-6 border-t border-slate-50 flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-widest">
          <span>Показване на {filteredPatients.length} от {patients.length} пациенти</span>
          <div className="flex items-center gap-2">
            <button className="p-2 rounded-lg hover:bg-slate-50 disabled:opacity-30" disabled>Назад</button>
            ...
            <button className="p-2 rounded-lg hover:bg-slate-50">Напред</button>
          </div>
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="px-8 pt-8 pb-4 flex items-center justify-between">
                <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Преглед на пациента</h3>
                <button
                  onClick={() => setSelectedModalPatient(null)}
              ...
              <div className="px-8 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-y-6">
                  <div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Дата на раждане</span>
                    <span className="text-sm font-bold text-slate-900">
                      {selectedModalPatient.birth_date ? new Date(selectedModalPatient.birth_date!).toLocaleDateString('bg-BG', { day: '2-digit', month: 'short', year: 'numeric' }) : 'Неизвестна'}
                    </span>
                  </div>
                  <div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Пол</span>
                    <span className="text-sm font-bold text-slate-900 capitalize">{selectedModalPatient.gender === 'male' ? 'Мъж' : selectedModalPatient.gender === 'female' ? 'Жена' : 'Неизвестен'}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Епилепсия</span>
                    <span className="text-sm font-bold text-slate-900">
                      {selectedModalPatient.medical_history?.toLowerCase().includes('epilepsy') ? 'Да' : 'Няма данни'}
                    </span>
                  </div>
                  <div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Регистриран</span>
                    <span className="text-sm font-bold text-slate-900">
                      {new Date(selectedModalPatient.created_at).toLocaleDateString('bg-BG')}
                    </span>
                  </div>
                </div>
                ...
                {/* Mobile App Status */}
                <div>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Мобилно приложение</span>
                  <div className={clsx(
              ...
              <div className="mt-8 bg-slate-900 p-8 rounded-t-[2rem]">
                <h4 className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-3">Медицинско досие</h4>
                ...
                >
                  Всички бележки <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            </motion.div>
          </div>
